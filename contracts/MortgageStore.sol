...
contract MortgageStore {
    ...
    function MortgageStore() public {
        productIndex = 0;
    }

    event NewProduct(uint _productId, string _name, string _imageLink, string _descLink, uint _auctionStartTime, uint _auctionEndTime, uint _startPrice);

    function addProductToStore(string _name, string _imageLink, string _descLink, uint _auctionStartTime,
        uint _auctionEndTime, uint _startPrice) public
    {
        require(_auctionStartTime < _auctionEndTime);
        productIndex++;
        Product memory product = Product(productIndex, msg.sender, _name, _imageLink, _descLink, _auctionStartTime, _auctionEndTime, _startPrice, 0, 0, 0, 0, ProductStatus.Open);
        stores[msg.sender][productIndex] = product;
        productIdInStore[productIndex] = msg.sender;
        NewProduct(productIndex, _name, _imageLink, _descLink, _auctionStartTime, _auctionEndTime, _startPrice);
    }

    function getProduct(uint _productId) view public returns (uint, string, string, string, uint, uint, uint, ProductStatus) {
        Product memory product = stores[productIdInStore[_productId]][_productId];
        return (product.id, product.name, product.imageLink, product.descLink, product.auctionStartTime,
        product.auctionEndTime, product.startPrice, product.status);
    }
